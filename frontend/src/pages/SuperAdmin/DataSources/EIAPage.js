...
import { getEIAStats } from "@api/dataSourcesAPI";
...
const EIAPage = () => {
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const resp = await getEIAStats();
      setData(resp.data || []);
    } catch (e) {
      message.error("Failed to load EIA stats");
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = useMemo(
    () => [
      { title: "Respondent", dataIndex: "respondent", key: "respondent" },
  ...
  return (
    <div style={{ padding: 24 }}>
      <Space direction="vertical" size="large" style={{ width: "100%" }}>
        <Title level={3}>EIA</Title>
        <Card title="Summary">
          <Table rowKey={(r) => r.respondent + (r.min_hour_utc || "")} loading={loading} dataSource={data} columns={columns} pagination={{ pageSize: 20 }} />
        </Card>
      </Space>
    </div>
  );
